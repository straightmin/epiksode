...
export function isCommentApiResponse(value: unknown): value is CommentApiResponse {
  return (
    typeof value === 'object' &&
    value !== null &&
...
export function isPaginatedApiResponse<T>(value: unknown): value is PaginatedApiResponse<T> {
  return (
    typeof value === 'object' &&
    value !== null &&
...
export function extractResponseData<T>(response: unknown): T {
  ...
  const apiResponse = response as { success?: boolean; data?: T; message?: string };
...
export function extractPaginatedData<T>(response: unknown): { data: T[]; pagination: PaginationMeta } {
  try {
    const extracted = extractResponseData<{ data: T[]; pagination: PaginationMeta }>(response);
    return {
      data: extracted.data || [],
    ...
    console.warn('Failed to extract paginated data, returning empty result:', error);
    return {
      data: [],
      pagination: {
        page: 1,
