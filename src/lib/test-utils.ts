...
import { getImageUrl, validateImageUrl, testImageLoad } from '@/lib/image-utils';
...
export async function testImageProxyService(): Promise<{
    ...
    const baseUrl = process.env.NEXT_PUBLIC_IMAGE_BASE_URL || 'http://localhost:3001/api/images';
    ...
    try {
        ...
        return {
            available: false,
            baseUrl,
            error: error instanceof Error ? error.message : '이미지 프록시 서비스 연결 실패',
        };
    }
}
...
export async function benchmarkImageLoading(
    ...
    return {
        ...
        averageTime: Math.round(loadTimes.reduce((sum, time) => sum + time, 0) / loadTimes.length),
...
export async function diagnoseImageProxySystem(): Promise<{
    ...
    if (!proxyService.available) {
        recommendations.push('이미지 프록시 라우터가 올바르게 등록되었는지 확인하세요');
    }
    
    if (apiConnection.responseTime > 1000) {
        recommendations.push('API 응답 시간이 느립니다. 서버 성능을 확인하세요');
    }
    
    if (sampleImageTest && !sampleImageTest.success) {
        recommendations.push('샘플 이미지가 존재하지 않거나 권한 문제가 있을 수 있습니다');
    }
...
export function validateEnvironmentConfig(): {
    valid: boolean;
    variables: Record<string, { value: string | undefined; required: boolean; valid: boolean }>;
    issues: string[];
} {
    const envVars = {
        NEXT_PUBLIC_API_URL: {
...
export function exposeTestUtils() {
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
        ...
        console.log('🎮 이미지 테스트 유틸리티가 window.epiksodeImageTests에 노출되었습니다');
        console.log('사용 예시:');
