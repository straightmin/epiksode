...
export function validateCommentContent(content: string): {
    ...
    if (repetitionPattern.test(sanitized)) {
        errors.push('Content appears to be spam (excessive repetition detected)');
    }
 ...
 */
export function validateUserInput(input: string, type: 'comment' | 'username' | 'email' = 'comment'): {
    isValid: boolean;
    errors: string[];
    sanitized: string;
} {
...
export class SecurityMonitor {
    ...
    static logViolation(type: string, description: string, metadata?: Record<string, unknown>): void {
        ...
        if (process.env.NODE_ENV === 'production') {
            console.warn('Security violation:', { type, description, metadata });
        } else {
            console.log('Security violation (dev):', { type, description, metadata });
        }
    }
