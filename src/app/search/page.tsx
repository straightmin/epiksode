...
function SearchContent() {
    ...
    const [activeTab, setActiveTab] = useState<"all" | "photos" | "users">(
        "photos"
    );
    ...
    const performSearch = useCallback(
        async (
            searchQuery: string,
            sortOrder: "relevance" | "latest" | "popular",
            isRetry = false
        ) => {
            if (!searchQuery.trim()) {
                setSearchResults({
    ...
    const handleSortChange = useCallback(
        (newSortBy: "relevance" | "latest" | "popular") => {
            setSortBy(newSortBy);
            if (query.trim()) {
                // 디바운스 타이머 취소하고 즉시 검색
                if (debounceTimerRef.current) {
    ...
    const tabs = [
        {
            ...
            label: "사진",
            icon: Camera,
            count: searchResults.total.photos,
        },
        {
            key: "users" as const,
            label: "사용자",
            icon: User,
            count: searchResults.total.users,
        },
        {
            key: "all" as const,
            label: "전체",
            icon: Search,
            count: searchResults.total.photos + searchResults.total.users,
        },
    ];
                    ...
                    {query && (
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                                {(
                                    ["relevance", "latest", "popular"] as const
                                ).map((sort) => (
                                    <button
                                        key={sort}
                    ...
                    {!query ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <div className="space-y-2">
                                <button
                                    onClick={() =>
                                        performSearch(query, sortBy, true)
                                    }
