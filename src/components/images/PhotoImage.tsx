...
const PhotoImage: React.FC<PhotoImageProps> = ({
    ...
    const thumbnailUrl = useMemo(() => {
        return progressiveLoading && photoId
            ? getImageUrl(photoId, true)
            : undefined;
    ...
    const handleError = useCallback(() => {
        ...
        const error = new Error(
            `이미지 로드 실패: photoId=${photoId}, thumbnail=${thumbnail}`
        );
        debugImageUrl("이미지 로드 실패", {
            photoId,
            thumbnail,
            url: imageUrl,
            error,
 ...
 */
export const PhotoThumbnail: React.FC<Omit<PhotoImageProps, "thumbnail">> = (
    props
) => {
    // src prop이 있으면 직접 URL 사용, 없으면 프록시 사용
    const shouldUseProxy = !props.src && !!props.photoId;
    return <PhotoImage {...props} thumbnail={shouldUseProxy} />;
};

/**
 * 반응형 PhotoImage 컴포넌트
 */
export const ResponsivePhotoImage: React.FC<
    Omit<PhotoImageProps, "responsive">
> = (props) => <PhotoImage {...props} responsive={true} />;

/**
 * 점진적 로딩 PhotoImage 컴포넌트
 */
export const ProgressivePhotoImage: React.FC<
    Omit<PhotoImageProps, "progressiveLoading">
> = (props) => <PhotoImage {...props} progressiveLoading={true} />;
