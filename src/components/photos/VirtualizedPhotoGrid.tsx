...
const VirtualizedPhotoGrid: React.FC<VirtualizedPhotoGridProps> = memo(
    ({
        ...
        const photoColumns = useMemo(() => {
            const columns: PhotoDetail[][] = Array.from(
                { length: columnCount },
                () => []
            );
        ...
        const virtualizedData = useMemo(() => {
            const startIndex = Math.max(
                0,
                Math.floor(scrollTop / (itemHeight + 16)) - overscan
            );
            const endIndex = Math.min(
                Math.max(...photoColumns.map((col) => col.length)),
                Math.ceil((scrollTop + containerHeight) / (itemHeight + 16)) +
                    overscan
            );
        ...
        const renderPhotoColumn = useCallback(
            (columnPhotos: PhotoDetail[], columnIndex: number) => {
                ...
                return (
                    <div key={columnIndex} className="flex flex-col gap-4">
                        ...
                        {startIndex > 0 && (
                            <div
                                style={{
                                    height: startIndex * (itemHeight + 16),
                                }}
                            />
                        )}
                            ...
                            .map((photo) => (
                                ...
                                >
                                    <ErrorBoundary
                                        ...
                                        onError={(error) => {
                                            console.warn(
                                                `VirtualizedPhotoCard 에러 (ID: ${photo.id}):`,
                                                error
                                            );
                                        }}
                                        fallback={
                        ...
                        {endIndex < columnPhotos.length - 1 && (
                            <div
                                style={{
                                    height:
                                        (columnPhotos.length - endIndex - 1) *
                                        (itemHeight + 16),
                                }}
                            />
                        )}
