...
export function CommentForm({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!commentRateLimiter.isAllowed(userKey)) {
            ...
            announceToScreenReader(`댓글 작성 제한: ${Math.ceil(remainingTime / 1000)}초 후 다시 시도해주세요`);
        ...
        if (!validation.isValid) {
            setValidationErrors(validation.errors);
            announceToScreenReader('댓글 작성 실패: ' + validation.errors.join(', '));
            return;
        }
            ...
            <div className="flex space-x-3">
                ...
                <div className="flex-1">
                    <textarea
                        ...
                        aria-describedby={validationErrors.length > 0 ? "comment-errors" : "comment-hints"}
                    ...
                    {/* Validation Errors */}
                    {validationErrors.length > 0 && (
                        <div id="comment-errors" className="mt-2" role="alert" aria-live="polite">
                    ...
                    {isRateLimited && (
                        <div className="mt-2 p-2 bg-yellow-100 border border-yellow-300 rounded text-sm text-yellow-800" role="alert">
                            댓글 작성이 일시적으로 제한되었습니다. 잠시 후 다시 시도해주세요.
                        </div>
                    )}
