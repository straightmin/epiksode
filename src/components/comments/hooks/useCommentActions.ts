...
import { CommentDetail, CreateCommentRequest, ToggleLikeRequest } from '@/types';
...
interface UseCommentActionsReturn {
    ...
    updateComment: (id: number, data: Partial<CommentDetail>) => Promise<CommentDetail>;
    deleteComment: (id: number) => Promise<void>;
    toggleLike: (data: { commentId: number; photoId?: number | null; seriesId?: number | null }) => Promise<CommentDetail>;
...
export function useCommentActions({
    ...
    const createComment = useCallback(async (data: CreateCommentRequest): Promise<CommentDetail> => {
    ...
    const toggleLike = useCallback(async (data: {
        ...
        return executeAction('like', async () => {
            ...
            const mockUpdatedComment: CommentDetail = {
                ...
                parentId: null,
                author: { id: 0, username: '', bio: null, profileImageUrl: null, createdAt: '' },
    ...
    const createReply = useCallback(async (data: CreateCommentRequest): Promise<CommentDetail> => {
